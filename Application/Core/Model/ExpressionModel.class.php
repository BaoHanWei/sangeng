...
class ExpressionModel extends Model
{
    ...
    public function getExpression($pkg,$aPage){
        ...
        if($pkg == 'miniblog'){
            ...
            if ($pkg == 'mycollection') {
               $uid = is_login();
                ...
                $iex_id=$iex_log->where('uid='.$uid)->field('iexpression_id')->select();
                foreach($iex_id as $v){
                    $iexid[]=$v['iexpression_id'];
                }

                $map['id']=array('in',$iexid);
                $iexplist = $iexpression->where($map)->page($aPage,$r)->select();
               $totalCount = $iexpression->where($map)->count();
              //  $iexplist = $iexpression->page($aPage,$r)->select();
               // $totalCount = $iexpression->count();
                $temp['title'] = 'add_img';
                ...
                foreach ($iexplist as $v) {
                    $temp['title'] = $v['id'];
                    $temp['emotion'] = $pkg== 'miniblog' ? '[' .  $v['id'] . ']' : '[' .$v['id'] . ':' .$pkg . ']';
                    $temp['filename'] = $v['id'];
                    $temp['type'] =$pkg;
                ...
                $res = array();
                $filelist = $expression->where($map1)->page($aPage,$r)->select();
                if($filelist) {
                    $totalCount = $expression->where($map1)->count();
