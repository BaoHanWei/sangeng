...
class BlogController extends AdminController{
    ...
    public function blogCategory()
    {
        ...
        $tree = $this->blogCategoryModel->getTree(0, 'id,title,sort,pid,status');
        $builder->title(L('_CATEGORY_MANAGER_'))
            ->suggest(L('_CATEGORY_MANAGER_SUGGEST_'))
            ->buttonNew(U('Blog/add'))
            ->data($tree)
            ->display();
    }
    ...
    public function add($id = 0, $pid = 0)
    {
        ...
        if (IS_POST) {
            if ($this->blogCategoryModel->editData()) {
                S('SHOW_EDIT_BUTTON',null);
                $this->success($title.L('_SUCCESS_'), U('Blog/blogCategory'));
            } else {
                $this->error($title.L('_FAIL_').$this->blogCategoryModel->getError());
            }
        } else {
            $builder = new AdminConfigBuilder();
            if ($id != 0) {
                $data = $this->blogCategoryModel->find($id);
            } else {
                $father_category_pid=$this->blogCategoryModel->where(array('id'=>$pid))->getField('pid');
                if($father_category_pid!=0){
                    $this->error(L('_ERROR_CATEGORY_HIERARCHY_'));
                }
            }
            if($pid!=0){
                $categorys = $this->blogCategoryModel->where(array('pid'=>0,'status'=>array('egt',0)))->select();
            }
            $opt = array();
            ...
            $builder->title($title.L('_CATEGORY_'))
                ...
                ->buttonSubmit(U('Blog/add'))->buttonBack()
                ->display();
        }
    ...
    public function setStatus($ids, $status)
    {
        ...
        if($status==0||$status==-1){
            ...
            $this->blogModel->where($map)->setField('category',1);
        }
        $builder = new AdminListBuilder();
        $builder->doSetStatus('blogCategory', $ids, $status);
    }
//分类管理end
...
str;
    ...
    public function index($page=1,$r=20)
    {
        ...
        if($aCate){
            $cates=$this->blogCategoryModel->getCategoryList(array('pid'=>$aCate));
            if(count($cates)){
        ...
        list($list,$totalCount)=$this->blogModel->getListByPage($map,$page,'update_time desc','*',$r);
        $category=$this->blogCategoryModel->getCategoryList(array('status'=>array('egt',0)),1);
        ...
        $builder->title(L('_NEWS_LIST_'))
            ->data($list)
            ->setSelectPostUrl(U('Admin/Blog/index'))
            ...
            ->keyDoActionEdit('Blog/editBlog?id=###');
        if(!$aDead){
            $builder->ajaxButton(U('Blog/setDead'),'',L('_SET_EXPIRE_'))->keyDoAction('Blog/setDead?ids=###',L('_SET_EXPIRE_'));
        }
    ...
    public function audit($page=1,$r=20)
    {
        ...
        $category=$this->blogCategoryModel->getCategoryList(array('id'=>array('in',$cates),'status'=>1),1);
        $category=array_combine(array_column($category,'id'),$category);
        foreach($list as &$val){
            $val['category']='['.$val['category'].'] '.$category[$val['category']]['title'];
        }
        ...
        $builder->title(L('_AUDIT_LIST_'))
            ->data($list)
            ->setStatusUrl(U('Blog/setBlogStatus'))
            ->buttonEnable(null,L('_AUDIT_SUCCESS_'))
            ->buttonModalPopup(U('Blog/doAudit'),null,L('_AUDIT_UNSUCCESS_'),array('data-title'=>L('_AUDIT_FAIL_REASON_'),'target-form'=>'ids'))
            ->setSelectPostUrl(U('Admin/Blog/audit'))
    ...
    public function doAudit()
    {
        if(IS_POST){
            ...
            $res=$this->blogModel->where(array('id'=>array('in',$ids)))->setField(array('reason'=>$reason,'status'=>-1));
            if($res){
                $result['status']=1;
                $result['url']=U('Admin/Blog/audit');
                //发送消息
                $messageModel=D('Common/Message');
                foreach($ids as $val){
                    $blog=$this->blogModel->getData($val);
                    $tip = L('_YOUR_NEWS_').'【'.$blog['title'].'】'.L('_FAIL_AND_REASON_').$reason;
                    $messageModel->sendMessage($blog['uid'], L('_NEWS_AUDIT_FAIL_'),$tip,  'Blog/Index/detail',array('id'=>$val), is_login(), 2);
                }
                //发送消息 end
            }else{
                $result['status']=0;
            ...
            $ids=implode(',',$ids);
            $this->assign('ids',$ids);
            $this->display(T('Blog@Admin/audit'));
        }
    }

    public function setBlogStatus($ids,$status=1)
    {
        !is_array($ids)&&$ids=explode(',',$ids);
        $builder = new AdminListBuilder();
        S('blog_home_data',null);
        //发送消息
        $messageModel=D('Common/Message');
        foreach($ids as $val){
            $blog=$this->blogModel->getData($val);
            $tip = L('_YOUR_NEWS_').'【'.$blog['title'].'】'.L('_AUDIT_SUCCESS_').'。';
            $messageModel->sendMessage($blog['uid'],L('_NEWS_AUDIT_SUCCESS_'), $tip,  'Blog/Index/detail',array('id'=>$val), is_login(), 2);
        }
        //发送消息 end
        $builder->doSetStatus('Blog', $ids, $status);
    }
    ...
    public function editBlog()
    {
        ...
        if(IS_POST){
            ...
            $this->_checkOk($data);
            $result=$this->blogModel->editData($data);
            if($result){
                S('blog_home_data',null);
                $aId=$aId?$aId:$result;
                $this->success($title.L('_SUCCESS_'),U('Blog/editBlog',array('id'=>$aId)));
            }else{
                $this->error($title.L('_SUCCESS_'),$this->blogModel->getError());
            }
        }else{
            $position_options=$this->_getPositions();
            if($aId){
                $data=$this->blogModel->find($aId);
                $detail=$this->blogDetailModel->find($aId);
    ...
    {
        !is_array($ids)&&$ids=explode(',',$ids);
        $res=$this->blogModel->setDead($ids);
        if($res){
            //发送消息
            $messageModel=D('Common/Message');
            foreach($ids as $val){
                $blog=$this->blogModel->getData($val);
                $tip = L('_YOUR_NEWS_').'【'.$blog['title'].'】'.L('_SET_TO_EXPIRE_').'。';
                $messageModel->sendMessage($blog['uid'],L('_NEWS_TO_EXPIRE_'),  $tip, 'Blog/Index/detail',array('id'=>$val), is_login(), 2);
            }
            //发送消息 end
            S('blog_home_data',null);
            $this->success(L('_SUCCESS_TIP_'),U('Blog/index'));
        }else{
            $this->error(L('_OPERATE_FAIL_').$this->blogModel->getError());
        }
    }
...
4:本类推荐
str;
        $positons=modC('NEWS_SHOW_POSITION',$default_position,'Blog');
