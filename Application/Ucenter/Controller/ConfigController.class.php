...
1600:Lv7 '.L('_CHAIRMAN__').'
    ...
    public function tag()
    {
        ...
        if (IS_POST) {
            ...
            $job = I('post.job', '', 'op_t');
            $work_years = I('post.work_years', '', 'op_t');
            $number_1 = I('post.number_1', '', 'op_t');
            $number_2 = I('post.number_2', '', 'op_t');
            $result = $userTagLinkModel->editData($aTagIds,$job,$work_years,$number_1,$number_2);
            if ($result) {
                $res['status'] = 1;
            } else {
                $res['status'] = 0;
            ...
            $myOtherTags = $userTagLinkModel->getUserOtherTag(is_login());
            $this->assign('my_tag', $myTags);
            $this->assign('my_other_tags', $myOtherTags);
