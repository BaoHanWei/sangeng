...
class IndexController extends BaseController
{
    ...
    {
        $user_info = query_user(array('avatar128', 'nickname', 'uid', 'space_url', 'score', 'title', 'fans', 'following', 'weibocount', 'rank_link', 'signature','pos_province','pos_city','pos_district','reg_time','last_login_time'), $uid);
        //获取用户封面id
    ...
    public function verifyRule(){
        $uid = isset($uid) ? $uid : is_login();
        $rank_id=I('rank_id',0,'intval');
        $rankInfo=D('rank')->where(array('types'=>1,'id'=>$rank_id))->field('id,rule_content,title')->find();
    ...
    public function applyExpert(){
        ...
        $rank_id=I('rank_id',0,'intval');
        $rankInfo=D('rank')->where(array('types'=>1,'id'=>$rank_id))->field('id,rule_content,title')->find();
        $rankUser=D("rank_user")->where(array("rank_id"=>$rank_id,'uid'=>$uid))->field('id')->find();
        $this->assign('rankInfo', $rankInfo);
        $this->assign('data', $rankUser);
        $this->display();
    }
    ...
    public function verify($rank_id = null, $reason = null, $rank_user_id = 0)
    {
        $rank_id = intval($rank_id);
        $reason = op_t($reason);
        $data['truename']=I('post.truename','','op_t');
        $data['qq']=I('post.qq',0,'intval');
        $data['city']=I('post.city','','op_t');
        $data['company']=I('post.company','','op_t');
        $data['technology']=I('post.technology','','op_t');
        ...
        if ($result) {
            ...
            $this->success(L('_SUCCESS_RANK_APPLY_'), U('Ucenter/Index/rankwait'));
        } else {
            $this->error(L('_FAIL_RANK_APPLY_'));
        }
    }
    private function _checkOk($data=array()){
        if(!mb_strlen($data['truename'],'utf-8')){
            $this->error("真实姓名不能为空");
        }
        if(empty($data['qq'])){
            $this->error("QQ号不能为空");
        }
        if(!mb_strlen($data['city'],'utf-8')){
            $this->error("所在城市不能为空");
        }
        if(!mb_strlen($data['company'],'utf-8')){
            $this->error("所在公司不能为空");
        }
        if(!mb_strlen($data['technology'],'utf-8')){
            $this->error("最擅长的5种技术不能为空");
        }
        return true;
    }
     ...
     */
     public function column(){
        $uid = isset($uid) ? $uid : is_login();
        $project=M("blog_project")->where("uid=".$uid)->select();
        foreach ($project as &$val) {
            $val['logo_url'] = get_pic_src(M('picture')->where('id=' . $val['logo'])->field('path')->getField('path'));
            $val['description']=msubstr($val['description'],0,90);
        }
        unset($val);
     ...
     public function columnEdit(){
        ...
        if(is_login && !empty($rankInfo)){
            if (IS_POST) {
                $data['uid'] =is_login();
                $data['project_name']=I('post.project_name','','htmlspecialchars');
                $data['description'] =I('post.description','','text');
                $data['logo']=I('post.cover',0,"intval");
                $data['create_time']=date("Y-m-d H:i:s");
                $data['project_open_state']=1;
                $project_info=M("blog_project")->where("uid=".is_login()." and project_name='".$data['project_name']."'")->find();
                if(!$project_info){
                    $result=M("blog_project")->add($data);
                    if($result){
                        $this->success("添加专栏成功", U('Ucenter/Index/column'));
                    }else {
                        $info['status']=0;
                        $info['info']=M("blog_project")->getError();
                    }
                ...
                $category=M('blog_category')->where("status=1")->order("sort asc")->select();
                $this->assign('category',$category);
                $this->display("columnedit");
            }
         }else{
            $this->error("你没有权限新增专栏");
         }
    }
}