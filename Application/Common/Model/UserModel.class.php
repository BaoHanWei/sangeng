...
class UserModel
{
    ...
    function query_user($pFields = null, $uid = 0)
    {
        ...
        $user_data['reg_time']=$this->getRegTime($uid);
        $user_data['last_login_time']=$this->getLastLoginTime($uid);
        if ($user_data['pos_province'] != 0) {
            $user_data['province']=$this->getCityName($user_data['pos_province']);
            $user_data['city']=$this->getCityName($user_data['pos_city']);
            $user_data['district']=$this->getCityName($user_data['pos_district']);
...
//读取等级数据
    ...
    //根据城市编码获取城市名称
    private function getCityName($districtCode){
        $cityName=S('getCityName_'.$districtCode);
        if(empty($cityName)){
            $districtInfo=M('district')->where(array('id'=>$districtCode))->find();
            S('getCityName_'.$districtCode,$districtInfo['name']);
            $cityName=$districtInfo['name'];
        }
        return $cityName;
    }
    private function getRegTime($uid){
        $regTime=S('getRegTime'.$uid);
        if(empty($regTime)){
            $userInfo=M('member')->field('reg_time')->where(array('uid'=>$uid))->find();
            S('getRegTime'.$uid,$userInfo['reg_time']);
            $regTime=$userInfo['reg_time'];
        }
        return $regTime;
    }
    
    private function getLastLoginTime($uid){
        $lastLoginTime=S('getLastLoginTime'.$uid);
        if(empty($lastLoginTime)){
            $userInfo=M('member')->field('last_login_time')->where(array('uid'=>$uid))->find();
            S('getLastLoginTime'.$uid,$userInfo['last_login_time']);
