...
function parse_expression_callback($data)
{
    ...
    if (preg_match("/#.+#/i", $data[0])) {
        ...
        $pkgname = substr($str, strrpos($str, ':') + 1, strlen($str) - strrpos($str, ':') - 2);//mycollection
       $expname=substr($str,1,strrpos($str, ':')-1);//279
        if($pkgname=='mycollection'&&preg_match('/^[0-9]*$/',$expname)){
            $key=1;
        }
    else{
        $key=0;
        ...
        if ($info) {
            if ($key==0) {
                ...
                return preg_replace("/\\[.+?\\]/i", "<div class='expression_pic'><img class='expimg' src='" . $info['src'] . "'  onmouseover='showspan(this)' /><span class='collect' onclick='collect(this)'onmouseout='hidspan(this)'></span></div>", $data[0]);
            }
