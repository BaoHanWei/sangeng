    ...
    foreach ($at_users as $e) {
        ...
        if ($user) {
            ...
            if (modC('HIGH_LIGHT_AT', 1, 'Weibo') && !$disabel_hight) {
                $content = str_replace("@[$e]", " <a class='user-at hl ' ucard=\"$user[uid]\" href=\"$query_user[space_url]\"><img src=\"$query_user[avatar32]\">@$query_user[nickname] </a> ", $content);
            } else {
...
function parse_at_app_users($content, $disabel_hight = false)
{
    ...
    foreach ($at_users as $e) {
        ...
        if ($user) {
            ...
            if (modC('HIGH_LIGHT_AT', 1, 'Weibo') && !$disabel_hight) {
                $content = str_replace("@[$e]", " <a class='user-at' data-user-id=\"$user[uid]\">@$query_user[nickname] </a> ", $content);
            } else {
                $content = str_replace("@[$e]", " <a data-user-id=\"$user[uid]\">@$query_user[nickname] </a> ", $content);
            }
        }
    }
    //返回替换的文本
