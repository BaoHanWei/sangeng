...
(function (win) {
    ...
    win.editor = editormd("editormd", {
        path : "/public/docs/editormd/lib/",
        placeholder: "本编辑器支持Markdown编辑，左边编写，右边预览",
        imageUpload: true,
        imageFormats: ["jpg", "jpeg", "gif", "png", "JPG", "JPEG", "GIF", "PNG"],
        imageUploadURL: "/ucenter/document/upload",
        fileUpload: true,
        fileUploadURL : '/ucenter/document/upload',
        tocStartLevel : 1,
        tocm : true,
        ...
        toolbarHandlers :{
            ...
            back : function (cm, icon, cursor, selection) {
                if(document.referer){
                    ...
                    window.location = '/ucenter/index/column.html';
                }
    ...
    win.deleteDocumentDialog = function (node) {
        var index = layer.confirm('你确定要删除该文档吗？', {
            ...
            $.post("/ucenter/document/delete/id/" + node.id).done(function (res) {
                layer.close(index);
                if(res.errcode == 0){
    ...
    win.loadDocument = function (selected) {
        ...
        $.get("/ucenter/document/content/id/" + selected.node.id + '?dataType=json').done(function (data) {
            layer.close(index);
