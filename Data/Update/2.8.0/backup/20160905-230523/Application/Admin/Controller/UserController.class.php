...
class UserController extends AdminController
{
    ...
    public function submitNickname()
    {
        ...
        empty($aNickname) && $this->error(L('_PLEASE_ENTER_A_NICKNAME_'));
        empty($aPassword) && $this->error(L('_PLEASE_ENTER_THE_PASSWORD_'));

        //密码验证
        $User = new UserApi();
        $uid = $User->login(UID, $aPassword, 4);
    ...
    public function changeStatus($method = null)
    {
        ...
        switch (strtolower($method)) {
            ...
            case 'resumeuser':
                ...
                break;
            case 'deleteuser':
                M('Follow')->where(array('who_follow' or 'follow_who'=> $id))->delete();
                $data = array('status' => -1);
                M('ucenter_member')->where($map1)->save($data);
    ...
    public function editScoreType()
    {
        ...
        if (IS_POST) {
            ...
            $data['type_key']=I('post.type_key','','op_t');
            $data['type_value']=I('post.type_value','','op_t');
            ...
            $builder->title(($aId == 0 ? L('_NEW_') : L('_EDIT_')) . L('_INTEGRAL_CLASSIFICATION_'))->keyId()->keyText('title', L('_NAME_'))
                ...
                ->keySelect('type_key', '类型', '数据表的字段类型', array( 2 => 'float', 3 => 'double',4=>'decimal'))
                ->keyUserDefined('type_value', '', '', T('Admin@User/edit'),array('type_value'=>$type))
                ->data($type)
