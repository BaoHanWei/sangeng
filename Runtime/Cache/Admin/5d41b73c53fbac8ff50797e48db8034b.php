...
<div class="panel-main" style="float:left;">
    ...
    <div class="main-title">
        <h2><?php echo L("_BEHAVIOR_LIST_");?></h2>
    </div>

    <div class="with-padding">
        <div class="row">
            <div class="col-xs-6">
                <button class="btn" id="action_add" url="<?php echo U('user/addaction');?>"><?php echo L("_NEW_GROWTH_WITH_SPACE_");?></button>
                <button class="btn ajax-post" target-form="ids"
                        url="<?php echo U('setstatus',array('status'=>1,'Model'=>'action'));?>"><?php echo L("_KAI_WITH_SPACE_");?>
                </button>
                <button class="btn ajax-post" target-form="ids"
                        url="<?php echo U('setstatus',array('status'=>0,'Model'=>'action'));?>"><?php echo L("_BAN_WITH_SPACE_");?>
                </button>
                <button class="btn ajax-post confirm" target-form="ids"
                        url="<?php echo U('setStatus',array('status'=>-1,'Model'=>'action'));?>"><?php echo L("_DELETE_WITH_SPACE_");?>
                </button>
            </div>
            <div class="col-xs-6 text-right">
                <form action="/index.php?s=/admin/user/action.html" id="filter" method="post">
                    <div class="oneselect">
                        <div class="title"><?php echo L("_THE_MODULE_"); echo L("_COLON_");?></div>
                        <div class="select_box">
                            <select name="module" data-role="select_text" class="form-control"
                                    onchange="$('#filter').submit()">
                                <option value="-1"><?php echo L("_ALL_");?></option>
                                <?php if(is_array($module)): $i = 0; $__LIST__ = $module;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['name'] == $current_module): ?><option value="<?php echo ((isset($vo["name"]) && ($vo["name"] !== ""))?($vo["name"]):''); ?>" selected><?php echo ($vo["alias"]); ?></option>
                                        <?php else: ?>
                                        <option value="<?php echo ((isset($vo["name"]) && ($vo["name"] !== ""))?($vo["name"]):''); ?>"><?php echo ($vo["alias"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                            </select>
                        </div>
                    </div>
                </form>
    ...
    <div class="data-table with-padding">
        <table class="table table-bordered table-striped table-hover">
            ...
            <tr>
                ...
                <th class=""><?php echo L("_NO_");?></th>
                <th class=""><?php echo L("_IDENTIFICATION_");?></th>
                <th class=""><?php echo L("_NAME_");?></th>
                <th class=""><?php echo L("_THE_MODULE_");?></th>
                <th class=""><?php echo L("_INTEGRAL_CHANGE_");?></th>
                <th class=""><?php echo L("_TYPE_");?></th>
                <th class=""><?php echo L("_RULE_");?></th>
                <th class=""><?php echo L("_STATE_");?></th>
                <th class=""><?php echo L("_OPERATION_");?></th>
            </tr>
            </thead>
            <tbody>
            <?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <td><a href="<?php echo U('ActionLimit/limitlist',array('action'=>$vo['name']));?>" title=<?php echo L("_CHECK_THE_LIMIT_WITH_DOUBLE_");?>><?php echo ($vo["name"]); ?></a>
                    </td>
                    <td><a href="<?php echo U('editAction?id='.$vo['id']);?>"><?php echo ($vo["title"]); ?></a></td>
                    <td><?php echo ($vo["alias"]); ?></td>
                    <td><?php echo ($vo["vary"]); ?></td>
                    <td><span><?php echo get_action_type($vo['type']);?></span></td>
                    <td><?php echo ($vo["remark"]); ?></td>
                    <td><?php echo ($vo["status_text"]); ?></td>
                    <td><a href="<?php echo U('User/editAction?id='.$vo['id']);?>"><?php echo L("_EDIT_");?></a>
                        <a href="<?php echo U('User/setStatus?Model=action&ids='.$vo['id'].'&status='.abs(1-$vo['status']));?>"
                           class="ajax-get"><?php echo (show_status_op($vo["status"])); ?></a>
                        <a href="<?php echo U('User/setStatus?Model=action&status=-1&ids='.$vo['id']);?>" class="confirm ajax-get"><?php echo L("_DELETE_");?></a>
                    </td>
