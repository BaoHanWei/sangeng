...
<div class="panel-main" style="float:left;">
	...
	<div class="clearfix with-padding">
		...
		<div class="search-form col-md-offset-10 col-xs-2 ">
			<div class="input-group">
				<input type="text" name="nickname" class="search-input form-control" value="<?php echo I('nickname');?>" placeholder=<?php echo L("_PLEASE_ENTER_A_USER_NAME_OR_ID_WITH_DOUBLE_");?>>
				                   <span class="input-group-btn"><a class="btn" href="javascript:;" id="search" url="<?php echo U('userList');?>"><i class="icon-search"></i></a></span>
			</div>
		</div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td>
                <a href="<?php echo U('Rank/userRankList?id='.$vo['uid']);?>"><?php echo L("_USER_TITLE_LIST_");?></a>
                <a href="<?php echo U('Rank/userAddRank?id='.$vo['uid']);?>"><?php echo L("_ASSOCIATED_NEW_TITLE_");?></a>
            </td>
		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		<?php else: ?>
		<td colspan="9" class="text-center"> aOh!<?php echo L("_THERE_IS_NO_CONTENT_");?> </td><?php endif; ?>
	</tbody>
    </table>
	</div>
