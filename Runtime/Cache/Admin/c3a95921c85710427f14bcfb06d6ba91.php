...
<div class="panel-main" style="float:left;">
	...
	<div class="main-title">
		<h2><?php echo L('_AUTH_MANAGER_');?></h2>
	</div>

    <div class="tools auth-botton with-padding">
        <a id="add-group" class="btn" href="<?php echo U('createGroup');?>"><?php echo L('_ADD_WITH_SPACE_');?></a>
        <a url="<?php echo U('changestatus',array('method'=>'resumeGroup'));?>" class="btn ajax-post" target-form="ids" ><?php echo L('_ENABLE_WITH_SPACE_');?></a>
        <a url="<?php echo U('changestatus',array('method'=>'forbidGroup'));?>" class="btn ajax-post" target-form="ids" ><?php echo L('_DISABLE_WITH_SPACE_');?></a>
        <a  url="<?php echo U('changestatus',array('method'=>'deleteGroup'));?>" class="btn ajax-post confirm btn-danger" target-form="ids" ><?php echo L('_DELETE_WITH_SPACE_');?></a>
        &nbsp; &nbsp; &nbsp; &nbsp;
        <a class="btn" href="<?php echo U('Admin/menu/index');?>"><?php echo L('_MENU_BG_MANAGER_');?></a>
        <a class="btn btn-success" href="<?php echo U('Admin/menu/add');?>"><?php echo L('_MENU_NEW_');?></a>
        <a class="btn" href="<?php echo U('addNode');?>"><?php echo L('_AUTH_FG_NEW_');?></a>
    </div>
	<!-- 数据列表 -->
    ...
    <thead>
		...
		<th class=""><?php echo L('_USER_GROUP_');?></th>
		<th class=""><?php echo L('_DESC_');?></th>

		<th class=""><?php echo L('_AUTH_');?></th>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a class=" btn btn-danger btn-xs" href="<?php echo U('AuthManager/access?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" ><?php echo L('_AUTH_BG_MANAGER_');?></a>
                <a class=" btn btn-success btn-xs" href="<?php echo U('AuthManager/accessuser?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" ><?php echo L('_AUTH_FG_MANAGER_');?></a>
			<a class=" btn btn-warning btn-xs" href="<?php echo U('AuthManager/user?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" ><?php echo L('_MEMBER_AUTH_');?></a>
			</td>
			<td><?php echo ($vo["status_text"]); ?></td>
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('AuthManager/changeStatus?method=forbidGroup&id='.$vo['id']);?>" class="ajax-get"><?php echo L('_DISABLE_');?></a>
				<?php else: ?>
				<a href="<?php echo U('AuthManager/changeStatus?method=resumeGroup&id='.$vo['id']);?>" class="ajax-get"><?php echo L('_ENABLE_');?></a><?php endif; ?>
<!--修改内容开始-->
				<a href="<?php echo U('AuthManager/changeStatus?method=deleteGroup&id='.$vo['id']);?>" class="confirm ajax-get"><?php echo L('_DELETE_');?></a>
<!--修改内容结束-->
                </td>
		</tr><?php endforeach; endif; else: echo "" ;endif; ?>
		<?php else: ?>
		<td colspan="6" class="text-center"> <?php echo L('_ADMIN_USER_EMPTY_');?> </td><?php endif; ?>
	</tbody>
    </table>
