...
<div class="panel-main" style="float:left;">
    ...
    <div class="main-title">
        <h2><?php echo isset($info['id'])?L('_EDIT_WITH_SINGLE_'):L('_NEW_WITH_SINGLE_'); echo L("_FRONT_RIGHT_NODE_");?></h2>
    </div>
    <form action="/index.php?s=/admin/auth_manager/addnode/id/10036.html" method="post" class="form-horizontal">
        ...
        <div class="form-item">
            ...
            <div class="controls">
                <input type="text" class="text input-large   form-control form-input-width" name="title" value="<?php echo ((isset($info["title"]) && ($info["title"] !== ""))?($info["title"]):''); ?>">

            </div>
        </div>
        <div class="form-item">
            <label class="item-label"><?php echo L("_NODE_IDENTITY_");?><span class="check-tips" ><?php echo L("_USED_TO_DISTINGUISH_RIGHT_FROM_CODE_");?>，<span style="color: red"><?php echo L("_UNIQUE_IDENTIFIER_");?></span>）</span></label>
            <div class="controls">
                <input type="text" class="text input-large  form-control form-input-width" name="name" value="<?php echo ((isset($info["name"]) && ($info["name"] !== ""))?($info["name"]):''); ?>">

            </div>
        </div>

        <div class="form-item">
            <label class="item-label"><?php echo L("_THE_MODULE_");?><span class="check-tips"><?php echo L("_MODULES_OF_THE_RIGHT_NODE_");?></span></label>
            <div class="controls">
                <select name="module" class=" form-control form-select-size">
                    <?php if(is_array($Modules)): $i = 0; $__LIST__ = $Modules;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$module): $mod = ($i % 2 );++$i; if(($module["is_setup"]) != "0"): ?><option value="<?php echo ($module["name"]); ?>" <?php if(($module["name"]) == $info['module']): ?>selected<?php endif; ?>><?php echo ($module["alias"]); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
