			...
			<div class="container">
				<div class="layout-right edit-catalog">
					<div class="select-box">
							分类：
							<input class="drop-input" id="self_catalog" type="hidden" name="catalog" value="<?php echo $category[0]['id']; ?>">
							<span class="select-show over-hide" id="current_category" data-value="<?php echo $category[0]['id']; ?>"><?php if(!empty($category[0]['title'])){echo $category[0]['title'];}else{echo "暂无分类";} ?></span>
							<i class="icon-tri select-icon"></i>
							<div class="dropdown">
							<div class="sort-items">
								<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="select-opt self-select-opt" data-value="<?php echo ($vo["id"]); ?>">
							...
							<div class="select-box">
							       ...
							       <i class="icon-tri select-icon"></i>
							        <div class="dropdown" id="category">
							        	<?php if(is_array($blog_category)): $i = 0; $__LIST__ = $blog_category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="select-opt sys-select-opt" data-value="<?php echo ($vo["id"]); ?>"><?php $title=explode('_',$vo['title']);echo $title[0]; ?></div><?php endforeach; endif; else: echo "" ;endif; ?>
      								</div>
							 </div>
						</div>
					</li>
