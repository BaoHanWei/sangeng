  ...
  <!--导航栏-->
        <ul class="nav nav-pills nav-stacked" style="margin-top:7px;">
          <li id="side_collection">
            ...
            <li id="side_column">
              <?php if($uid == get_uid()): ?><a href="<?php echo U('Ucenter/Index/column');?>"><?php echo L('_I_');?>的专栏<span class="pull-right"></span></a>
               <?php else: ?> 
                <a href="<?php echo U('Ucenter/Index/column/',array('uid'=>$uid));?>"><?php echo L('_TA_');?>的专栏<span class="pull-right"></span></a><?php endif; ?>  
            </li>
        </ul>
</div>
    <div class="panels-default panel-skills">
        	 ...
        	 <div class="col-xs-9 main-article">
        	 		<div class="panels-heading" id="panels-heading-pjax">
				        <nav class="title sub-nav clearfix">
				            <a href="javascript:;" class="active" id="dynamic-all">专栏列表</a>
                    <?php if($uid == get_uid()): ?><a href="<?php echo U('ucenter/index/columnEdit');?>"" class="btn btn-default btn-green-border btn-edit no-select" style="font-size:14px;line-height:22px;float:right;background:#3EB158;color:#fff;text-shadow:0 0 0 #fff;margin-top:5px;margin-right:-2px"><i class="icon-svg icon-pen-green"></i>新增专栏</a><?php endif; ?>
				        </nav>
				    </div>
                    <div class="project-list"> 
                        <?php if(is_array($project)): $i = 0; $__LIST__ = $project;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="box">
                                 <strong><a href="<?php echo U('ucenter/document/index',array('id'=>$vo['id']));?>" target="_blank"><?php echo ($vo["project_name"]); ?></a></strong>
                                 <dl class="clearfix">
                                 <?php if(!empty($vo["logo_url"])): ?><dt><a href="/xfsc/lxzs/14491.html" target="_blank"><img src="<?php echo ($vo["logo_url"]); ?>" title="<?php echo ($vo["project_name"]); ?>" alt="<?php echo ($vo["project_name"]); ?>"></a></dt><?php endif; ?>
                                     <dd><p class="intro"><?php echo ($vo["description"]); ?></p>
                                     <p class="info">
                                        <span title="" data-toggle="tooltip" data-placement="bottom" data-original-title="创建时间"><i class="icon icon-time"></i> 2017/02/08 16:17</span>
                                       <span style="display: inline-block;padding-left: 10px;" title="创建者"><i class="icon icon-user"></i>admin</span> 
                                       <span style="display: inline-block;padding-left: 10px;" title="文档数量"><i class="icon icon-file"></i> 1</span> 
                                       <span style="display: inline-block;padding-left: 10px;" title="项目角色"><i class="icon icon-group"></i> 拥有者 </span> 
                                       <span style="display: inline-block;padding-left: 10px;" title="最后编辑"><i class="icon icon-pencil"></i> 最后编辑: admin 于 2017-02-08 16:17</span> 
                                    </p>
                                    </dd>
                                 </dl>
                             </div><?php endforeach; endif; else: echo "" ;endif; ?>
