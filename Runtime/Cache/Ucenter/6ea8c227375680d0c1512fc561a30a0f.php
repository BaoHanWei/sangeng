...
<div class="with-padding" style="padding: 20px">
    <ul class="nav nav-secondary">
        <li class="active"><a href="#base" data-toggle="tab"> <?php echo L('_AVATAR_MODIFY2_');?></a></li>
     </ul>
    <?php $user = query_user(array('avatar128')); ?>
    <div class="row">
        <div class="ucenter-avatar col-xs-12">
            <?php if(file_exists('./api/uc_login.lock')){ include_once './api/uc_client/client.php'; $html = uc_avatar(is_login()); ?>
            <?php echo ($html); ?>
            <?php }else{ ?>
            <?php echo W('Ucenter/UploadAvatar/render',array('uid'=>is_login()));?>
            <?php } ?>
        </div>
    </div>
</div>
