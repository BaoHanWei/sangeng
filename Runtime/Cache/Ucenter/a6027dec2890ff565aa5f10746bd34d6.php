...
<script src="/Public/js.php?f=js/jquery-2.0.3.min.js,js/com/com.functions.js,js/com/com.toast.class.js,js/com/com.ucard.js,js/core.js"></script>
    ...
    <link type="text/css" href="/Application/Ucenter/Static/css/blog/blog.css" rel="stylesheet">
        ...
        <div id="container" class="container">
            <table style="width: 100%; table-layout: fixed;">
                ...
                <tr id="usercenter-sidebar-sm">
                    <td style="width: 25%; vertical-align: top; background-color: #fff;border-right:1px dashed #d1d1d1;min-height:700px" id="usercenter-sidebar-td">
                       <?php if(CONTROLLER_NAME == Blog): $sideBarUser = query_user(array('nickname','space_url', 'uid', 'title', 'score', 'avatar128'), $uid); $call = $call ? $call : L('_I_'); ?>
<div class="row">
    <div class="col-xs-12">
        <div class="with-padding">
        	 <div>
                 <div class="user-info-panel text-center margin_bottom_10">
                     <div>
                         <a href="<?php echo ($sideBarUser["space_url"]); ?>" target="_blank"><img class="avatar-img" src="<?php echo ($sideBarUser["avatar128"]); ?>"></a>
                     </div>
                     <div style="margin-top:10px;">
                         <a class="nickname" href="<?php echo ($sideBarUser["space_url"]); ?>" style="color:#333;font-size:17px;"><?php echo ($sideBarUser["nickname"]); ?></a>
                         <a href="<?php echo ($sideBarUser["space_url"]); ?>" target="_blank">
           ...
           <?php else: ?>
	            <div class="edit">
		            <a href="<?php echo U('Ucenter/Blog/edit');?>" class="btn-green btn-lg">写 博 客</a>
		        </div><?php endif; ?>
	        <div style="margin-top:40px;">
	            ...
	            <ul class="nav nav-pills nav-stacked">
	            	<?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li id="side_<?php echo ($vo["id"]); ?>" <?php if($catid==$vo['id']){echo 'class="active"';} ?>>
		                    <a href="<?php echo U('Ucenter/Blog/index',array('catid'=>$vo['id']));?>">
		                       <?php echo ($vo["title"]); ?>
		                        <span class="pull-right"><?php echo ($vo["count"]); ?></span>
		                    </a>
		                </li><?php endforeach; endif; else: echo "" ;endif; ?>
	            </ul>
            </div>
            <?php if(is_login() && $uid == get_uid()): ?><div>
	            <h3 class="with-padding" style="border-bottom:1px solid #e1e1e1;">博客管理</h3>
	            <!--导航栏-->
	            <ul class="nav nav-pills nav-stacked">
	                <li id="side_blog_catalogs">
	                    <a href="<?php echo U('Ucenter/Blog/blog_catalogs');?>">分类管理<span class="icon icon-node pull-right"></span></a>
	                </li>
	                <li id="side_blog_comments">
	                    <a href="<?php echo U('Ucenter/Blog/blog_comments');?>">评论管理<span class="icon icon-comments-alt pull-right"></span></a>
	                </li>
	                <script>
...
<div class="row ">
    <div class="col-xs-12">
        <div class="with-padding">
            ...
            <h3 class="with-padding" style="border-bottom:1px solid #e1e1e1;">分类</h3>
            <!--导航栏-->
            <ul class="nav nav-pills nav-stacked">
                <li id="side_message">
                ...
                <script>
                    ...
                    <td id="usercenter-content-td"
                        style="width: 75%; background-color: #fff; vertical-align: top; padding-left: 50px; padding-right: 50px;">
        ...
        <div class="col-xs-12 blog-user">
            <div class="navbar flex-item">
			    <div class="flex-item-7 flex-item-md-7">
			        <div class="nav-tabs">
			            <a class="nav-tab <?php if($type=='newblog'){echo 'active';} ?>" href="<?php echo U('Ucenter/Blog/index');?>">最新</a>
			            <a class="nav-tab <?php if($type=='hotblog'){echo 'active';} ?>" href="<?php echo U('Ucenter/Blog/index',array('type'=>'hotblog'));?>">最热</a>
			        </div>
			    </div>
			    <div class="flex-item-5 flex-item-md-5">
			        <form id="searchForm" method="get" action="?" onsubmit="return false;">
			            <div class="search">
			                ...
			                <input type="text" name="search" id="search" placeholder="#> 搜索博客" value="<?php echo $_GET['search']; ?>">
			                <div class="enter"></div>
			            </div>
			        </form>
			    </div>
    ...
    <div style="min-height: 750px;" class="blog-user">
        <?php if(!$list): ?><p class="text-muted" style="text-align: center; font-size:14px;">
            <br><br>
            	暂无博客哦
            <br><br><br>
        </p><?php endif; ?>
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vl): $mod = ($i % 2 );++$i;?><div class="list-item">
                  <div class="layout">
                       <div class="layout-column">
                          <div class="title">
                              <?php if(($vl["blog_type"]) == "1"): ?><span class="status-tag original" title="原创博客"> 原</span><?php endif; ?>
                              <?php if(($vl["blog_type"]) == "2"): ?><span class="status-tag reprint" title="转载博客"> 转</span><?php endif; ?>
                              <a href="<?php echo U('Blog/Index/detail',array('id'=>$vl[id]));?>" class="blog-title">
                              	<?php echo ($vl["title"]); ?>
                               	<?php if(($vl["is_top"]) == "1"): ?><span class="status-stick text-orange" title="置顶博客">::置顶</span><?php endif; ?>
                               </a>
                          </div>
                          <div class="data-info text-gary">
                               <ul>
                               	  <li class="read"><?php echo L('_PUBLISH_'); echo L('_COLON_'); echo (friendlydate($vl["create_time"])); ?></li>
                                  <li class="read">阅读:<span id="read"><?php echo ($vl["view"]); ?></span></li>
                                  <li class="comment">评论: <span id="comment"><?php echo ($vl["comment"]); ?></span></li>
                                  <li class="like">赞:<span id="like"><?php echo ($vl["praise"]); ?></span></li>
                                  <li class="favor">收藏: <span id="favor"><?php echo ($vl["collection"]); ?></span></li>
                                 <?php if(is_login() && $uid == get_uid()): ?><li>(<span class="edit"><a href="<?php echo U('Ucenter/Blog/editBlog',array('id'=>$vl[id]));?>">编辑</a></span>&nbsp;|&nbsp;<span class="delete"><a href="javascript:;" bolg-id="<?php echo ($vl["id"]); ?>" cid="<?php echo ($vl["cid"]); ?>" class="delete_blog">删除</a></span>)</li><?php endif; ?>
                                </ul>
                          </div>
                      </div>
                  </div>
    ...
    <script type="text/javascript" src="/Application/Ucenter/Static/js/blog/blog_edit.min.js"></script>
