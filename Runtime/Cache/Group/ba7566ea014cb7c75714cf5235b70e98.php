...
<?php echo hook('pageHeader');?>
    <script type="text/javascript" src="/Application/Group/Static/js/common.js"></script>
    <link type="text/css" rel="stylesheet" href="/Application/Group/Static/css/group.css"/>
    <script>
...
<div class="main-wrapper">
    ...
    <div class="group_header_block ">
        <div class="col-md-4 hidden-xs hidden-sm">
            ...
            <?php }else{ ?>
            <img class="logo" src="/Application/Group/Static/images/default.png" alt="<?php echo (op_t($group["title"])); ?>">
            <?php } ?>
        </div>
...
<?php if($list_top): ?><div class="clearfix fourm-posts common_block_border">
        <div class="row common_block_title">
            <?php echo L('_TOP_AREA_');?>
        </div>
        <div class="col-xs-12">
            <section id="contents">
                <div style="padding: 0 5px;">
                <?php if(is_array($list_top)): $i = 0; $__LIST__ = $list_top;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$document): $mod = ($i % 2 );++$i;?><a class="group-list-title-link" title="<?php echo (htmlspecialchars($document["title"])); ?>" href="<?php echo U('Index/detail',array('id'=>$document['id']));?>">
                        ...
                        <img src="/Application/Group/Static/images/is_top.png">
                    </a>
                    <?php if($i != count($list_top)): ?><hr class=""/>
                        <?php else: ?>
...
<div class="clearfix fourm-posts common_block_border">
    <div class="clearfix group_block_title" style="font-size: 14px;padding: 2px 5px">
        <div class="group_tab pull-left">
            <ul id="" class="nav nav-tabs" role="tablist">
                <li
                <?php if($type == 'post'): ?>class="active"<?php endif; ?>
                ><a href="<?php echo U('group',array('type'=>'post','id'=>$group_id));?>"><?php echo L('_POST_');?></a></li>
                <li
                <?php if($type == 'new'): ?>class="active"<?php endif; ?>
                ><a href="<?php echo U('group',array('type'=>'new','id'=>$group_id));?>"><?php echo L('_RECENT_NEWS_');?></a></li>
                <li
                <?php if($type == 'member'): ?>class="active"<?php endif; ?>
                ><a href="<?php echo U('group',array('type'=>'member','id'=>$group_id));?>"><?php echo L('_MEMBER_');?></a></li>
            </ul>
        </div>
        <div class="pull-right" style="display: inline-block;width: 175px">
            <form action="<?php echo U('group',array('id'=>$group_id));?>" method="post">
                <div class="group_post_search input-group ">
                    <input placeholder="<?php echo L('_PLACE_HOLDER_SEARCH_');?>" type="text" name="title" class="form-control" value="<?php echo ($search_key); ?>">
    ...
    <?php if($type == 'new'){ ?>
        <div class="">
            <div class="common_block_content group_feed">
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dynamic): $mod = ($i % 2 );++$i; echo W('dynamic/lists',array('dynamic_id'=>$dynamic)); endforeach; endif; else: echo "" ;endif; ?>
           <?php if(!$list): ?><div class="row">
                        <div class="col-xs-12">
                            <p class="text-muted" style="text-align: center; font-size: 3em;">
                                <br/><br/>
                                <?php echo L('_NEW_LACK_');?>
                                <br/><br/><br/>
                            </p>
                        </div>
                    </div><?php endif; ?>
    ...
    <?php }elseif($type == 'post'){ ?>
        ...
        <div class="">
            <div class="clearfix common_block_title">
                <div class="col-xs-8 group_tags group_types ">
                    <a <?php if(empty($_GET['cate'])): ?>class="active"<?php endif; ?> href="<?php echo U('group',array('id'=>$group_id));?>" style="font-size: 14px;"><?php echo L('_ALL_');?></a>
                    <?php if(is_array($post_cate)): $i = 0; $__LIST__ = $post_cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><a <?php if($cate['id'] == $_GET['cate']): ?>class="active"<?php endif; ?> href="<?php echo U('group',array('cate'=>$cate['id'],'id'=>$group_id));?>" style="font-size: 14px;"><?php echo ($cate["title"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                ...
                <div class="col-xs-4 text-right">
                    <div class="btn-group group_order" >
                        <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">
                            <?php if(($order) == "1"): echo L('_REPLY_'); echo L('_TIME_');?>
                                <?php else: ?>
                                <?php echo L('_POSTING_'); echo L('_TIME_'); endif; ?>
                            <span class="caret"></span>
                        </button>
                        <ul class="dropdown-menu text-left" role="menu">
                            <li><a href="<?php echo U('group',array('id'=>$group_id,'order'=>'ctime'));?>"><?php echo L('_POSTING_'); echo L('_TIME_');?></a></li>
                            <li><a href="<?php echo U('group',array('id'=>$group_id,'order'=>'reply'));?>"><?php echo L('_REPLY_'); echo L('_TIME_');?></a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <section id="contents">
                    <?php if(!$list): ?><div class="row">
                            <div class="col-xs-12">
                                <p class="text-muted" style="text-align: center; font-size: 3em;">
                                    <br/><br/>
                                    <?php echo L('_POST_LACK_');?>
                                    <br/><br/><br/>
                                </p>
                            </div>
                        </div><?php endif; ?>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; echo W('Group/Detail/post',array('post_id'=>$vo));?>
        ...
        <div class="col-xs-12">
            <?php if(is_joined($group_id) == 1): if(check_auth('Group/Index/groupInvite',-1)){ ?>
            <a class="group_invite btn btn-primary" href="<?php echo U('group/Index/groupInvite',array('group_id'=>$group_id));?>"><?php echo L('_INVITE_FRIENDS_');?></a>
                <?php } endif; ?>
            <div style="margin: 20px 0 20px 20px;">
                <h2 class="clearfix"><?php echo L('_MEMBER_');?></h2>
                <?php if(count($list) == 0): ?><p class="text-muted"
                                                     style="text-align: center; font-size: 3em;">
                    <br><br>
                    <?php echo L('_MEMBER_LACK_');?>
                    <br><br><br>
                </p><?php endif; ?>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$people): $mod = ($i % 2 );++$i;?><div class="col-xs-2 text-center" style="padding-bottom: 20px;height: 140px;">
                        <img ucard="<?php echo ($people["user"]["uid"]); ?>"  class="avatar-img"
                             src="<?php echo ($people["user"]["avatar128"]); ?>"/>
                        <a href="<?php echo ($people["user"]["space_url"]); ?>"
                           ucard="<?php echo ($people["user"]["uid"]); ?>"><?php echo ($people["user"]["nickname"]); ?></a>

                        <div>
                            <?php if($people['position'] == 3 ){ echo L('_CREATER_'); } ?>
                            <?php if($people['position'] == 2 ){ echo L('_MANAGER_'); } ?>
                        </div>

                    </div><?php endforeach; endif; else: echo "" ;endif; ?>
                <div class="pull-right">
                    <?php echo getPagination($totalCount,30);?>
                </div>
            </div>
        </div>
