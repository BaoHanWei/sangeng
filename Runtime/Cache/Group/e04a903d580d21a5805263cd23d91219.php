...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div class="common_block_border group_info margin-bottom-15">
        <div class="group_tags group_types" style="padding: 15px;">
            <a href="<?php echo U('groups');?>"
            <?php if(!$group_cate): ?>class="active"<?php endif; ?>
            >
            <!--//TODO 添加标签的链接-->
            <?php echo L('_ALL_');?>
            </a>
            <?php if(is_array($parent)): $i = 0; $__LIST__ = $parent;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><a href="<?php echo U('groups',array('cate'=>$type['id']));?>" data-id="<?php echo ($type['id']); ?>" class="ptype <?php if( $group_cate == $type['id']): ?>active<?php endif; ?>">
                <!--//TODO 添加标签的链接-->
                <?php echo ($type["title"]); ?>
    ...
    <?php if(is_array($child)): $i = 0; $__LIST__ = $child;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(!empty($vo)){ ?>
        <div class="common_block_border group_info chile_type" id="type_<?php echo ($key); ?>" style="margin-top: -10px; ">
            <div class="group_tags group_types" style="padding: 15px;">
                <a href="<?php echo U('groups',array('cate'=>$key));?>"
                <?php if(!$group_cate): ?>class="active"<?php endif; ?>
                >
                <!--//TODO 添加标签的链接-->
                <?php echo L('_ALL_');?>
                </a>
                <?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><a href="<?php echo U('groups',array('cate'=>$type['id']));?>"
                    <?php if($group_cate == $type['id']): ?>class="active"<?php endif; ?>
                    >
                    <!--//TODO 添加标签的链接-->
                    <?php echo ($type["title"]); ?>
                    </a><?php endforeach; endif; else: echo "" ;endif; ?>
            ...
            <div>
                <div class="">
                    <section class="group_lists">
                        ...
                        <?php if(is_array($group_list)): $i = 0; $__LIST__ = $group_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; echo W('Group/Detail/group',array('group_id'=>$vo)); endforeach; endif; else: echo "" ;endif; ?>

                        <div class="item_page clearfix pull-right">
