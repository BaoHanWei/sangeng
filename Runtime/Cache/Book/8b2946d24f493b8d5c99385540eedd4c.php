...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div id="wrapper">
        <div id="navsecond">
            ...
            <ul>
                <li class="design"><a href="<?php echo U('tutorials');?>"
                <?php if(!$group_cate): ?>class="active"<?php endif; ?>
                >
                <?php echo L('_ALL_');?>
                </a></li>
                <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><li class="design"><a href="<?php echo U('tutorials',array('cate'=>$type['id']));?>" data-id="<?php echo ($type['id']); ?>" class="ptype <?php if( $group_cate == $type['id']): ?>active<?php endif; ?>">
                    <?php echo ($type["title"]); ?>
        ...
        <div id="project-content">
            <?php if(is_array($bookList)): $i = 0; $__LIST__ = $bookList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="pro-list" style="display: block;">
                <div class="item-title">
                  <h2><?php echo ($vo["title"]); ?></h2></div>
                <div class="pro-list-content">
                  <?php if($vo['books'] != null): if(is_array($vo["books"])): $i = 0; $__LIST__ = $vo["books"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="pro-item">
                        <a href="<?php echo U('Book/index/read',array('id'=>$val['id']));?>" title="<?php echo ($val["title"]); ?>">
                          <div class="pro-item-img">
                            <img src="<?php echo (getthumbimagebyid($val["img"],300,300)); ?>" style="" alt="<?php echo ($val["title"]); ?>" title="<?php echo ($val["title"]); ?>"></div>
                          <div class="pro-item-desc">
                            <h4><?php echo ($val["title"]); ?></h4>
                            <p><?php echo mb_substr(text($val['summary']), 0, 30, 'utf-8'); ?>...</p>
                          </div>
                        </a>
