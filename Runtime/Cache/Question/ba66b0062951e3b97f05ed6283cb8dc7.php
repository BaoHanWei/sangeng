        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="common_block_border clearfix">
                    <div style="margin:0 10px;border-bottom: 1px solid #ddd;">
                        <ul class="nav nav-pills">
                            <li <?php if(($type) == "q"): ?>class="question_active"<?php endif; ?>>
                                <a href="<?php echo U('Question/Index/myQuestion',array('type'=>'q'));?>" style="font-size:16px;">
                                  <?php echo L("_MY_PROBLEM_WITH_SPACE_");?>
                                </a>
                            </li>
                            <li <?php if(($type) == "a"): ?>class="question_active"<?php endif; ?>>
                                <a href="<?php echo U('Question/Index/myQuestion',array('type'=>'a'));?>" style="font-size:16px;">
                                  <?php echo L("_MY_ANSWER_WITH_SPACE_");?>
                                </a>
                            </li>
                        </ul>
                    </div>
                    <div class="question_list" style="padding:0 20px">
                        <?php if(!empty($list)): if($type == 'q'): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><section class="stream-list__item">
   <div class="qa-rank">
        <div class="votes hidden-xs">
            <?php echo ($data["good_question"]); ?><small>投票</small>
        </div>
        <?php if($data['answer_num'] == 0): ?><div class="answers">
	            <?php echo ($data["answer_num"]); ?><small>回答</small>
	        </div><?php endif; ?>
	     <?php if(($data['answer_num'] != 0) and ($data['best_answer'] == 0)): ?><div class="answers answered">
	            <?php echo ($data["answer_num"]); ?><small>回答</small>
	        </div><?php endif; ?>
	     <?php if(($data['answer_num'] != 0) and ($data['best_answer'] > 0)): ?><div class="answers answered solved">
	            <?php echo ($data["answer_num"]); ?><small>解决</small>
	        </div><?php endif; ?>
    ...
    <div class="summary">
        <ul class="author list-inline">
              <li><a href="<?php echo ($data["user"]["space_url"]); ?>"><?php echo ($data["user"]["nickname"]); ?></a><span class="split"></span>
              		<a href="<?php echo U('Question/Index/detail',array('id'=>$data['id']));?>" class="askDate"><?php echo (friendlydate($data["create_time"])); ?></a>
              </li>
        </ul>
        <h2 class="title"><a href="<?php echo U('Question/Index/detail',array('id'=>$data['id']));?>"><?php echo ($data["title"]); ?></a></h2>
        <?php if(!empty($data['tags'])){ ?>
        <ul class="taglist--inline ib">
        	<?php $tags=explode(',',$data['tags']);for($i=0;$i<count($tags);$i++){ ?>
            <li class="tagPopup"><a class="tag tag-sm" href="<?php echo U('Question/Index/tag',array('name'=>$tags[$i]));?>"><?php echo $tags[$i]; ?></a></li>    
            <?php } ?>
        </ul>
        <?php } ?>
     </div>
                                ...
                                <?php else: ?>
                                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><section class="stream-list__item">
   <div class="qa-rank">
        <div class="votes hidden-xs">
            <?php echo ($data["question"]["good_question"]); ?><small>投票</small>
        </div>
        <?php if($data['question']['answer_num'] == 0): ?><div class="answers">
	            <?php echo ($data["question"]["answer_num"]); ?><small>回答</small>
	        </div><?php endif; ?>
	     <?php if(($data['question']['answer_num'] != 0) and ($data['question']['best_answer'] == 0)): ?><div class="answers answered">
	            <?php echo ($data["question"]["answer_num"]); ?><small>回答</small>
	        </div><?php endif; ?>
	     <?php if(($data['question']['answer_num'] != 0) and ($data['question']['best_answer'] > 0)): ?><div class="answers answered solved">
	            <?php echo ($data["question"]["answer_num"]); ?><small>解决</small>
	        </div><?php endif; ?>
        <div class="views hidden-xs viewsword0to99">
            <span><?php echo ($data["question"]["views"]); ?></span>
            <small>浏览</small>
        </div>
    </div>
    <div class="summary">
        <ul class="author list-inline">
              <li><a href="<?php echo ($data["question"]["user"]["space_url"]); ?>"><?php echo ($data["question"]["user"]["nickname"]); ?></a><span class="split"></span>
              		<a href="<?php echo U('Question/Index/detail',array('id'=>$data['id']));?>" class="askDate"><?php echo (friendlydate($data["question"]["create_time"])); ?></a>
              </li>
        </ul>
        <h2 class="title"><a href="<?php echo U('Question/Index/detail',array('id'=>$data['question']['id']));?>"><?php echo ($data["question"]["title"]); ?></a></h2>
        <?php if(!empty($data['question']['tags'])){ ?>
        <ul class="taglist--inline ib">
        	<?php $tags=explode(',',$data['question']['tags']);for($i=0;$i<count($tags);$i++){ ?>
            <li class="tagPopup"><a class="tag tag-sm" href="<?php echo U('Question/Index/tag',array('name'=>$tags[$i]));?>"><?php echo $tags[$i]; ?></a></li>    
            <?php } ?>
        </ul>
        <?php } ?>
                            ...
                            <?php else: ?>
                            <div style="font-size:1em;padding:2em 0;color: #ccc;text-align: center">暂时没有提任何问题吆~</div><?php endif; ?>
