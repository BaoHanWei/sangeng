...
<script src="/Public/js.php?f=js/jquery-2.0.3.min.js,js/com/com.functions.js,js/com/com.toast.class.js,js/com/com.ucard.js,js/core.js"></script>
    ...
    <link href="/Application/Question/Static/css/question.css" rel="stylesheet" type="text/css"/>
...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div class="col-xs-12">
        <div id="frm-post-popup" class="white-popup1 boxShadowBorder col-xs-9" style="border:1px solid #fff">
            <h2>我要<?php echo ($edit_title); ?></h2>
            <div class="aline" style="margin-bottom: 10px"></div>
            <div class="row">
                <div class="col-xs-12" style="margin-top: 30px">
                    <form class="form-horizontal ajax-form" role="form" action="<?php echo U('Question/Index/edit');?>" method="post">
                        <input type="hidden" name="id" id="id" value="<?php echo ($data["id"]); ?>"/>
                        <div class="form-group has-feedback">
                            <label for="title" class="col-xs-2 control-label"><?php echo L("_TITLE_");?></label>
                            <div class="col-xs-8">
                                <input id="title" name="title" class="form-control form_check" check-type="Text"value="<?php echo ($data["title"]); ?>" placeholder="标题：一句话说清问题，用问号结尾"/>                            </div>
                        </div>
                        <div class="form-group">
                            <label for="category" class="col-xs-2 control-label"><?php echo L("_CLASSIFICATION_");?></label>
                            <div class="col-xs-5">
                                <select id="category" name="category" class="form-control">
                                    <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$top): $mod = ($i % 2 );++$i;?><option value="<?php echo ($top["id"]); ?>"
                                        <?php if(($data['category']) == $top['id']): ?>selected<?php endif; ?>
                                        >
                                        <?php echo ($top["title"]); ?>
                        ...
                        <div class="form-group has-feedback">
                            ...
                            <div class="col-xs-8">
                                <input id="title" name="tags" class="form-control form_check" check-type="Text"value="<?php echo ($data["tags"]); ?>" placeholder="标签，如：php 多个可使用逗号,分隔"/>
                            </div>
                        </div>
                        <div class="form-group" style="display:none">
                            <label for="category" class="col-xs-2 control-label"> 悬赏</label>
                            <div class="col-xs-5">
                                <?php $field = D('Ucenter/Score')->getTypeList(array('status' => 1)); ?>
                                <select name="leixing" class="form-control">
                                    <?php if(is_array($field)): $i = 0; $__LIST__ = $field;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$leixing): $mod = ($i % 2 );++$i;?><option value="<?php echo ($leixing["id"]); ?>"><?php echo ($leixing["title"]); ?> </option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select>
                            </div>
                            <div class="col-xs-2">
                                <select name="score_num" class="form-control">
                                    <option value="0">悬赏0</option>
                                    <option value="5">悬赏5</option>
                                    <option value="10">悬赏10</option>
                                    <option value="15">悬赏15</option>
                                    <option value="20">悬赏20</option>
                                    <option value="30">悬赏30</option>
                                    <option value="50">悬赏50</option>
                                    <option value="80">悬赏80</option>
                                    <option value="100">悬赏100</option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-xs-2 control-label"><?php echo L("_SUPPLEMENTARY_DESCRIPTION_");?></label>
                            <div class="col-xs-10">
                                <?php $config="toolbars:[['source','|','bold','italic','underline','fontsize','forecolor','justifyleft','fontfamily','|','map','emotion','insertimage','insertcode']],initialContent:'<div class=\'default_css\'>1. 描述你的问题<br/><br/>2. 贴上相关代码<br/><br/>3. 贴上报错信息<br/><br/>4. 贴上相关截图<br/><br/>5.已经尝试过哪些方法仍然没解决（附上相关链接）</div>',autoClearinitialContent:true"; ?>
                                <?php echo W('Common/Ueditor/editor',array('myeditor_edit','description',$data['description'],'660px','250px',$config));?>                            </div>
                        </div>
                        <div class="form-group">
        ...
        <div class="col-xs-3" style="padding-right:0px;">
            <div class="boxShadowBorder" style="border:1px solid #fff;padding: 10px;font-size: 14px;line-height: 30px;color: #BBB;">
                <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo L("_PLEASE_IN_THE_SEARCH_BOX_ABOVE_SEARCH_HAVE_A_LOOK_THERE_IS_NO_ASSOCIATED_PROBLEM_BEFORE_ASKING_A_QUESTION_YO_WITH_WAVE_");?></p>               <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo L("_YOU_MIGHT_HAVE_THE_ANSWER_YOU_WANT_");?>^_^~~</p>            </div>
        </div>
    </div>
