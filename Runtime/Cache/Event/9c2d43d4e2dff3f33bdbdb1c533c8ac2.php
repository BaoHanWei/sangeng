            ...
            <div class="clearfix">
                <div class="detail-title">
                    <div class="col-xs-10 text-more">
                        <a href="javascript:" onclick="location.href='<?php echo U('Event/Index/detail',array('id'=>$content['id']));?>'" title="<?php echo ($content["title"]); ?>"><?php echo ($content["title"]); ?></a>
                    </div>
                    <div class="col-xs-2">
                ...
                <div class="col-xs-5 box-left">
                    <a class="pull-left" href="javascript:" onclick="location.href='<?php echo U('Event/Index/detail',array('id'=>$content['id']));?>'">
                        <img src="<?php echo (getthumbimagebyid($content["cover_id"],345,250)); ?>"/>
                       <?php
 $cover_path=getThumbImageById($content['cover_id']); ?>
                ...
                <div class="col-xs-6 box-right">
                    <div class="event_fq_count pull-left">
                        <div class="box-name">
                            <?php echo L('_SPONSOR_'); echo L('_COLON_');?>
                            <a class="event_count" href="<?php echo ($content["user"]["space_url"]); ?>"><?php echo ($content["user"]["nickname"]); ?></a>
                        </div>

                        <div class="mgt14 event_count" ><?php echo L('_TIME_'); echo L('_COLON_');?>
                            <?php echo date('Y-m-d',$content['sTime']);?>--<?php echo date('Y-m-d',$content['eTime']);?>
                        </div>

                        <div class="mgt14 event_count"><?php echo L('_SITE_'); echo L('_COLON_');?>
                            <?php echo ($content["address"]); ?>
                        </div>

                        <div class="mgt14 event_count"><?php echo L('_DEADLINE_TIME_'); echo L('_COLON_');?>
                            <?php echo date('Y-m-d H:i',$content['deadline']);?>
                        </div>

                        <div class="pull-left clearfix box-number">
                            <div class=" pull-left  mgt14">
                                <span class="event_count"><?php echo L('_NUMBER_LIMIT_'); echo L('_COLON_'); echo ($content["limitCount"]); echo L('_PEER_');?></span>
                            </div>


                            <div class=" pull-left   mgt14">
                                <span class="event_count"><?php echo L('_REGISTER_ED_'); echo L('_COLON_'); echo ($content["signCount"]); echo L('_PEER_');?></span>
                            </div>

                            <div class=" pull-left   mgt14">
                                <span class="event_count"><?php echo L('_ALREADY_IN_'); echo L('_COLON_'); echo ($content["attentionCount"]); echo L('_PEER_');?></span>
                            </div>
                        </div>
                        <div class="pull-left  mgt14">
                            <div class="btn-group">
                                ...
                                <?php if(time() < $content['eTime']): ?><a id="endEvent" class="btn btn-default"><?php echo L('_OVER_AHEAD_');?></a><?php endif; ?>

                                <a href="<?php echo U('edit',array('id'=>$content['id']));?>" class="btn btn-default"><?php echo L('_EVENT_EDIT_');?></a>
                                <a id="delEvent" class="btn btn-default"><?php echo L('_EVENT_DELETE_');?></a>
                                <?php } if($content['uid'] != is_login()){ if(!$check_isSign){ if($content['deadline'] < time()){ ?>
                                <a class="btn btn-default  " href="javascript:"
                                   onclick="toast.error(<?php echo L('');?>)"><?php echo L('_REGISTRATION_HAS_OVER_');?> </a>
                                <?php }else{ ?>
                                <?php if(is_login() == 0): ?><a class="btn  btn-info   " href="javascript:"style="color: #fff"
                                       onclick="toast.error(<?php echo L('_ERROR_REGISTER_AFTER_LOGIN_');?>)"><?php echo L('_REGISTER_OP_');?> </a>
                                    <?php else: ?>
                                    <?php if(check_auth('Event/Index/doSign')): ?><a class="btn  btn-info  event_sign " style="color: #fff"
                                           href="<?php echo U('ajax_sign',array('event_id'=>$content['id']));?>"><?php echo L('_REGISTER_OP_');?> </a><?php endif; endif; ?>
                                <?php } }else{ ?>
                                <a class="btn btn-default" href="javascript:"
                                   id="unSign"><?php echo L('_REGISTER_CANCEL_');?> </a>

                                <span style="line-height: 34px;float: left"> &nbsp;&nbsp;  <?php echo L('_AUDIT_ED_');?>&nbsp;&nbsp;<?php if($check_isSign[0]['status'] == 1): echo L('_AUDIT_ED_');?>
                                    <?php else: ?>
            ...
            <div class="title-box">
                <?php echo L('_EVENT_DETAIL_');?>
                <div class="share-box">
                    <div class="pull-right">
                        <?php echo W('Common/Share/detailShare');?>
                    </div>
                    <span class="pull-right">
                    <?php echo W('Weibo/Share/shareBtn',array('param'=>array('img'=>$cover_path,'title'=>$content['title'],'content'=>$content['explain'],'from'=>$MODULE_ALIAS,'site_link'=>U('Event/Index/detail',array('id'=>$content['id']))),'text'=>"站内分享"));?>
                    </span>
                </div>
            </div>

            <div class="word-wrap detail">
                <?php echo (render($content["explain"])); ?>

                <div class="pull-right time">
                    <?php echo L('_TIME_RELEASE_'); echo L('_COLON_');?> <?php echo (friendlydate($content["create_time"])); ?> &nbsp;&nbsp;
                    <?php echo L('_TIME_UPDATE_'); echo L('_COLON_');?> <?php echo (friendlydate($content["update_time"])); ?>
                </div>
            ...
            <div class="comment-box">
                <?php echo hook('localComment', array('path'=>"Event/event/$content[id]", 'uid'=>$content['uid'],'count_model'=>'event','count_field'=>'reply_count','this_url'=>'event/index/detail'));?>
            </div>
        </div>
    </div>
